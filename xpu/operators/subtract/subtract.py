...
def subtract_triton(a, b):
    ...
    # 启动内核
    subtract_kernel[grid](a, b, output, n_elements, BLOCK_SIZE)
    
    return output

def subtract_cpu(a, b):
    # 确保输入是numpy数组
...
def main():
    ...
    for size in sizes:
        ...
        cpu_result = subtract_cpu(a_np, b_np)
        ...
        # 预热
        _ = subtract_pytorch(a_torch[:1024], b_torch[:1024])
        
        start_time = time.time()
        pytorch_result = subtract_pytorch(a_torch, b_torch)
        pytorch_time = time.time() - start_time
        print(f"PyTorch GPU time: {pytorch_time:.6f} seconds")
        
        # GPU Triton测试
        # 预热
        _ = subtract_triton(a_torch[:1024], b_torch[:1024])
        
        start_time = time.time()
        triton_result = subtract_triton(a_torch, b_torch)
