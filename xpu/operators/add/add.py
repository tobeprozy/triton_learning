...
def add_triton(a, b):
    ...
    # 启动内核
    add_kernel[grid](a, b, output, n_elements, BLOCK_SIZE)
...
def main():
    ...
    for size in sizes:
        ...
        # 预热
        _ = add_pytorch(a_torch[:1024], b_torch[:1024])
        
        start_time = time.time()
        pytorch_result = add_pytorch(a_torch, b_torch)
        pytorch_time = time.time() - start_time
        print(f"PyTorch GPU time: {pytorch_time:.6f} seconds")
        
        # GPU Triton测试
        # 预热
        _ = add_triton(a_torch[:1024], b_torch[:1024])
        
        start_time = time.time()
        triton_result = add_triton(a_torch, b_torch)
